...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db, c = get_db()
            c.execute('insert into todo (description, completed, created_by) '
                      ' values (%s, %s, %s)',
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'select t.id, t.description, t.completed, t.created_by, '
        't.created_at, u.username from todo t join user u on t.created_by = u.id '
        'where t.id = %s',
        (id,)
    )
    ...
    if todo is None:
        abort(404, "the todo is not available")
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if not description:
            error = 'Not description available.'
